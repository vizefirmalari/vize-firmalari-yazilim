...
type Props = {
  ...
  /** URL ?hizmet= ile açılış (bir kez) — geçici olarak WhatsApp bilgi hattı */
...
export function GrowthServiceCard({
  ...
  useEffect(() => {
    if (!autoOpenToken || autoOpenToken !== service.id || !service.is_active) return;
    const a = document.createElement("a");
    a.href = GROWTH_SALES_WHATSAPP_INQUIRY_URL;
    a.target = "_blank";
  ...
  }, [autoOpenToken, onAutoOpenConsumed, service.id, service.is_active]);
    ...
    >
      ...
      <div className="mt-4 flex flex-col gap-2.5 sm:flex-row sm:items-stretch">
        {service.is_active ? (
          ...
          <span className="inline-flex min-h-11 flex-1 cursor-not-allowed items-center justify-center rounded-xl bg-[#0B3C5D]/35 px-4 text-sm font-semibold text-white">
            Satın Al
          </span>
        )}
        <Link
